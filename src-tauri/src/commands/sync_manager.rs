...
use crate::commands::gmail_integration::{ProcessedGmailMessage, store_parsed_message_in_db};
use crate::commands::email_parser::parse_gmail_api_message;
...
async fn fetch_and_process_message(
    ...
    let gmail_api_message: crate::commands::email_parser::GmailApiMessage = 
        serde_json::from_value(gmail_message.clone())?;
    let parsed_email = parse_gmail_api_message(&gmail_api_message)?;
    
    // Convert to ProcessedGmailMessage format
