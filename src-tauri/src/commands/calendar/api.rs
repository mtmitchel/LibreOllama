...
pub async fn create_calendar_event(
    ...
    let event_id = format!("event-{}", chrono::Utc::now().timestamp_millis());

    let mut new_event = event_data;
    new_event.id = event_id.clone();
    new_event.created = Some(chrono::Utc::now().to_rfc3339());
    new_event.updated = Some(chrono::Utc::now().to_rfc3339());
    new_event.status = Some("confirmed".to_string());

    println!("✅ [CALENDAR-API] Event created successfully: {}", event_id);
    Ok(new_event)
}
...
pub async fn update_calendar_event(
    ...
    updated_event.id = event_id.clone();
    updated_event.updated = Some(chrono::Utc::now().to_rfc3339());
