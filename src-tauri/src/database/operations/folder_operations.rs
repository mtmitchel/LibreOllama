...
pub fn create_folder(
    ...
    conn.execute(
        "INSERT INTO folders (name, parent_id, user_id, color, created_at, updated_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
        params![folder_name, parent_id, user_id, color, now, now],
    )?;
    let folder_id = conn.last_insert_rowid() as i32;
    get_folder(conn, folder_id).and_then(|f| f.ok_or_else(|| rusqlite::Error::QueryReturnedNoRows))
}
...
pub fn get_folder(conn: &Connection, folder_id: i32) -> Result<Option<Folder>> {
    ...
    let folder = stmt.query_row(params![folder_id], |row| {
        Ok(Folder {
            id: row.get(0)?,
            name: row.get(1)?,  // Changed from folder_name to name
            parent_id: row.get(2)?,
            user_id: row.get(3)?,
            color: row.get(4)?,
...
pub fn get_folders_by_user(conn: &Connection, user_id: &str) -> Result<Vec<Folder>> {
    let mut stmt = conn.prepare(
        "SELECT id, name, parent_id, user_id, color, created_at, updated_at 
         FROM folders WHERE user_id = ?1 ORDER BY name ASC"
    )?;
...
pub fn get_root_folders_by_user(conn: &Connection, user_id: &str) -> Result<Vec<Folder>> {
    let mut stmt = conn.prepare(
        "SELECT id, name, parent_id, user_id, color, created_at, updated_at 
         FROM folders WHERE user_id = ?1 AND parent_id IS NULL ORDER BY name ASC"
    )?;
...
pub fn get_folder_hierarchy(conn: &Connection, user_id: &str) -> Result<Vec<Folder>> {
    let mut stmt = conn.prepare(
        "WITH RECURSIVE folder_hierarchy AS (
            SELECT id, name, parent_id, user_id, color, created_at, updated_at, 0 as level
            FROM folders 
            WHERE user_id = ?1 AND parent_id IS NULL
            
            UNION ALL
            
            SELECT f.id, f.name, f.parent_id, f.user_id, f.color, f.created_at, f.updated_at, fh.level + 1
            FROM folders f
