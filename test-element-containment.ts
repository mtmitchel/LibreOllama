...
import { useCanvasStore } from './src/features/canvas/stores/canvasStore';
import type { CanvasElement } from './src/types';

// Test utilities
const delay = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

const createTestElement = (id: string, x: number, y: number): CanvasElement => ({
  id,
  type: 'rectangle',
  x,
  y,
...
// Test Suite
export const runElementContainmentTests = async () => {
...
if (typeof window !== 'undefined') {
  (window as any).runElementContainmentTests = runElementContainmentTests;
  (window as any).logCanvasState = logCanvasState;
