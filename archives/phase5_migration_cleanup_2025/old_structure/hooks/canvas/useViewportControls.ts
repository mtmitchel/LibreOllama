...
import { useViewport, useKonvaCanvasStore } from '../stores/konvaCanvasStore';

/**
 * useViewportControls - Zoom/pan controls from viewport store
 * - Integrates with viewport store for zoom and pan operations
...
export const useViewportControls = () => {
  ...
  const zoomToPoint = useCallback((zoomLevel: number, point: { x: number; y: number }) => {
    // For now, use basic setZoom - can be enhanced to support center point later
    setZoom(zoomLevel);
    // TODO: Implement center point zooming when store interface is enhanced
  }, [setZoom]);
