...
import { CanvasElement, useKonvaCanvasStore } from '../../features/canvas/stores/konvaCanvasStore';
import { TableCellData, TableRowData, TableColumnData } from '../../models/tableDataModel';
import { TableColumn, TableRow } from '../../features/canvas/stores/types';
...
// Type guard to check if a cell is a full TableCellData or just a simple text cell
const isFullTableCell = (cell: any): cell is TableCellData => {
...
export const EnhancedTableElement = React.forwardRef<Konva.Group, EnhancedTableElementProps>(
  ...
  const tableRows = (enhancedTableData.rows || []) as TableRowData[];
  const tableColumns = (enhancedTableData.columns || []) as TableColumnData[];
    ...
    const handleMouseUp = () => {
      ...
      if (resizeHandleRef.current === 'col' || resizeHandleRef.current === 'row') {
        ...
        if (currentElement?.enhancedTableData) {
          ...
          const currentColumns = (currentTableData.columns || []) as TableColumnData[];
          const currentRows = (currentTableData.rows || []) as TableRowData[];
          ...
          if (resizeHandleRef.current?.includes('e') && !resizeHandleRef.current?.includes('s') && !resizeHandleRef.current?.includes('n')) {
            ...
            updateElement(element.id, {
              ...
              enhancedTableData: {
                ...currentTableData,
                columns: updatedColumns as TableColumn[],
              },
            });
    ...
    try {
      return tableRows.map((row, rowIndex) =>
        tableColumns.map((col, colIndex) => {
          ...
          const cellText = (cellData as TableCellData)?.text || '';
            ...
            <Group key={`${rowIndex}-${colIndex}`} id={`${element.id}-cell-${rowIndex}-${colIndex}`} x={cellX} y={cellY}>
              ...
              {isFullTableCell(cellData) && cellData?.segments && cellData.segments.length > 0 ? (
                ...
                <Text
                  ...
                  text={cellText}
                  fontSize={isFullTableCell(cellData) ? cellData?.fontSize || 14 : 14}
                  ...
                  fill={isFullTableCell(cellData) ? cellData?.textColor || designSystem.colors.secondary[800] : designSystem.colors.secondary[800]}
