...
// Import types from the centralized types file to avoid duplication
import type {
  CanvasElement,
  TableCell,
  TableRow,
...
export type { RichTextSegment, CanvasElement };
...
const mergeSegments = (segments: RichTextSegment[]): RichTextSegment[] => {
  ...
  for (let i = 1; i < segments.length; i++) {
    ...
    const { text: _, ...currentStyle } = currentSegment;
    const { text: __, ...nextStyle } = nextSegment!;
...
// Remove duplicate CanvasElement interface - use the one from types.ts
...
export const useKonvaCanvasStore = create<CanvasState>()(
  immer((set, get) => ({
    ...
    addElement: (element) => {
      ...
      set((state) => {
        ...
        if (element.sectionId && state.sections[element.sectionId]) {
          ...
          if (section && !section.containedElementIds.includes(element.id)) {
    ...
    deleteElement: (id) => {
      ...
      set((state) => {
        if (section) {
          // Handle section deletion inline
          delete state.sections[id];
          // Remove the section from all contained elements
      ...
      get().addToHistory(`Delete ${element?.type || section?.type || 'element'}`);
    },
        ...
        section.containedElementIds.forEach(containedId => {
          ...
          if (containedElement) {
            ...
            set((state) => {
              ...
              if (newSection) {
                newSection.containedElementIds.push(newContainedId);
              }
            });
          }
        });
      } else {
        // Regular element duplication - inline the addElement logic
        set((state) => {
          state.elements[duplicatedElement.id] = duplicatedElement;
    ...
    clearCanvas: () => {
      // Clear all elements and sections
      set((state) => {
        state.elements = {};
        state.sections = {};
        state.selectedElementId = null;
      ...
      try {
        set((state) => {
          ...
          if (element.type === 'text') {
            ...
            const { text: _, ...elementWithoutText } = element;
          ...
          if (!richTextElement || richTextElement.type !== 'rich-text' || !richTextElement.segments) return;
    ...
    setIsEditingText: (id) => {
      // Inline the setEditingTextId logic to avoid recursive calls
      set((state) => {
        state.editingTextId = id;
        state.isEditingText = id; // Keep alias in sync
        if (id !== null) {
    ...
    updateElementText: (elementId, newText) => {
      set((state) => {
        ...
        if (element) {
          if (element.type === 'text') {
          ...
          } else if (element.type === 'rich-text' && element.segments && element.segments[0]) {
            ...
            const { text: _, ...firstSegmentStyle } = element.segments[0];

            element.segments = [{ ...firstSegmentStyle, text: newText }];
            // Remove text property for rich-text elements
            delete (element as any).text;
          }
        }
      });
      ...
      set((state) => {
        ...
        if (targetElement) {
          if (newSection) {
            ...
            targetElement.x = relativeCoords.x;
            targetElement.y = relativeCoords.y;
            targetElement.sectionId = newSection.id;
            ...
            targetElement.x = absoluteX;
            targetElement.y = absoluteY;
            targetElement.sectionId = null;
          }
        }
      });
    },
    ...
    handleSectionDragEnd: (sectionId: string, newPosition: { x: number; y: number }) => {
      ...
      set((state) => {
        ...
        if (section && !section.isLocked) {
          ...
          section.x = newPosition.x;
          section.y = newPosition.y;
    ...
    createSectionFromTemplate: (templateId: string, position: { x: number; y: number }) => {
      ...
      template.elements.forEach((templateElement: any, index: number) => {
        ...
        set((state) => {
          ...
          if (state.sections[sectionId]?.containedElementIds) {
    ...
    setElementSection: (elementId: string, sectionId: string | null, newRelativePos?: { x: number; y: number }) => {
      set((state) => {
        ...
        if (sectionId) {
          // Add to new section
          const section = state.sections[sectionId];
          if (section?.containedElementIds && !section.containedElementIds.includes(elementId)) {
            section.containedElementIds.push(elementId);
          }
    ...
    createEnhancedTable: (x: number, y: number, rows?: number, cols?: number) => {
      ...
      const enhancedTableData: EnhancedTableData = {
        ...
        showGridLines: tableDataModel.showGridLines ?? true,
        cornerRadius: tableDataModel.cornerRadius ?? 0,
        borderWidth: tableDataModel.borderWidth ?? 1,
        borderColor: tableDataModel.borderColor ?? '#E5E7EB',
        defaultCellPadding: tableDataModel.defaultCellPadding ?? 8,
        autoResizeRows: tableDataModel.autoResizeRows ?? true,
        allowDragAndDrop: tableDataModel.allowDragAndDrop ?? true,
        keyboardNavigationEnabled: tableDataModel.keyboardNavigationEnabled ?? true
      };
    ...
    updateTableCell: (tableId: string, rowIndex: number, colIndex: number, updates: Partial<TableCell>) => {
      ...
      set((state) => {
        ...
        if (oldElement) {
          state.elements[tableId] = { ...oldElement, enhancedTableData: element.enhancedTableData } as CanvasElement;
          ...
          console.log('🏪 [STORE DEBUG] Updated element structure:', {
            ...
            hasEnhancedTableData: !!element.enhancedTableData,
            cellCount: element.enhancedTableData?.cells?.length
          });
        }
    ...
    addTableRow: (tableId: string, insertIndex?: number) => {
      set((state) => {
        ...
        const newCells: TableCell[] = enhancedTableData.columns.map((_, colIndex) => {
          ...
          const tempTable = TableDataModelUtils.createTable(tableId, 1, 1);
          const cellData = tempTable.cells[0]?.[0];
    ...
    addTableColumn: (tableId: string, insertIndex?: number) => {
      set((state) => {
        ...
        enhancedTableData.cells.forEach((row, rowIndex) => {
          ...
          const newCell: TableCell = cellData ? {
            ...cellData,
            id: cellId,
            isHeader,
    ...
    removeTableRow: (tableId: string, rowIndex: number) => {
      set((state) => {
        ...
        const rowCells = enhancedTableData.cells[rowIndex];
        if (rowCells) {
    ...
    setTableSelection: (tableId: string, selection: TableSelection | null) => {
      set((state) => {
        ...
        if (selection) {
          element.enhancedTableData.selection = selection;
        } else {
          delete element.enhancedTableData.selection;
        }
      });
    },
    ...
    serializeTableCell: (tableId: string, rowIndex: number, colIndex: number): string | null => {
      ...
      return TableCellDataUtils.serialize({
        ...cell,
    ...
    cloneTableCell: (tableId: string, rowIndex: number, colIndex: number, targetTableId: string, targetRowIndex: number, targetColIndex: number): boolean => {
      ...
      const clonedCell = TableCellDataUtils.clone({
        ...sourceCell,
        segments: sourceCell.segments || []
      }, newCellId);
    ...
    upgradeTableToNewFormat: (tableId: string): boolean => {
      ...
      if (element.enhancedTableData) {
        ...
        element.enhancedTableData.cells.forEach((row, rowIndex) => {
          row.forEach((cell, colIndex) => {
            const upgradedCell = TableCellDataUtils.upgradeToRichText({
              ...cell,
              segments: cell.segments || []
            });
            if (JSON.stringify(upgradedCell) !== JSON.stringify(cell)) {
              const targetRow = element.enhancedTableData?.cells[rowIndex];
              if (targetRow) {
                targetRow[colIndex] = upgradedCell;
                hasChanges = true;
              }
            }
          });
      ...
      for (let rowIndex = 0; rowIndex < rows; rowIndex++) {
        for (let colIndex = 0; colIndex < cols; colIndex++) {
          ...
          if (legacyText) {
            ...
            const targetRow = newTableData.cells[rowIndex];
            if (targetRow) {
              targetRow[colIndex] = upgradedCell;
            }
          }
      ...
      set((state) => {
        ...
        if (el && el.type === 'table') {
          el.enhancedTableData = {
            rows: newTableData.rows.map(row => ({
              ...row,
              isResizable: row.isResizable ?? true
            })) as TableRow[],
            columns: newTableData.columns.map(col => ({
              ...col,
              isResizable: col.isResizable ?? true
            })) as TableColumn[],
            cells: newTableData.cells.map(row =>
              row.map(cell => ({
                ...cell,
                segments: cell.segments || []
              }))
            ) as TableCell[][],
            showGridLines: newTableData.showGridLines ?? true,
            cornerRadius: newTableData.cornerRadius ?? 0,
            borderWidth: newTableData.borderWidth ?? 1,
            borderColor: newTableData.borderColor ?? '#E5E7EB',
            defaultCellPadding: newTableData.defaultCellPadding ?? 8,
            autoResizeRows: newTableData.autoResizeRows ?? true,
            allowDragAndDrop: newTableData.allowDragAndDrop ?? true,
            keyboardNavigationEnabled: newTableData.keyboardNavigationEnabled ?? true
          };
        }
      });
