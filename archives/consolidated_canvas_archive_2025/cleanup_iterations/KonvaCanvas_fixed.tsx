...
const KonvaCanvas: React.FC<KonvaCanvasProps> = ({
  ...
  // Get elements from store instead of local state
  ...
  // FIXED: Simplified canvas click handler - ONLY handles selection/deselection
  const handleStageClick = useCallback((e: any) => {
    const stage = e.target.getStage();
    
    console.log('🎯 Stage click - tool:', selectedTool, 'target:', e.target.getClassName(), 'targetId:', e.target.id());

    // FIXED: Better empty space detection
    const clickedOnEmpty = e.target === stage;
    
    console.log('🎯 Clicked on empty space?', clickedOnEmpty, 'target:', e.target.getClassName());
    ...
    // If we get here, user clicked on an existing element - handle selection
    console.log('🎯 Clicked on existing element - handling selection');
  }, [setSelectedElement, selectedTool]);

  // SEPARATE function for creating elements - only called from toolbar
  const createElementAtPosition = useCallback((toolType: string, position: { x: number; y: number }) => {
    console.log('🚀 Creating new element with tool:', toolType, 'at position:', position);
    ...
    console.log('📊 Store state after creation:', {
      totalElements: Object.keys(elements).length + 1,
      newElementId: newElement.id
    });
  }, [addElement, setSelectedElement, elements]);

  // Expose create function to parent component
  useEffect(() => {
    if (onCreateElement) {
      // Store the function reference for toolbar to call
      (window as any).createCanvasElement = (toolType: string) => {
        const centerPosition = {
          x: width / 2,
          y: height / 2
        };
        createElementAtPosition(toolType, centerPosition);
      };
    }
  }, [createElementAtPosition, onCreateElement, width, height]);
    ...
    >
      ...
      {elementArray.length === 0 && (
        <div style={{
          ...
          🎨 Canvas ready! Select a tool and click to create elements
        </div>
      )}
