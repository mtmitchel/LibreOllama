...
describe('Unified Canvas Store: Sections and Containment', () => {
  ...
  describe('Section Creation & Management', () => {
    it('should create a section with specified properties', async () => {
      const { getState } = store;
      let sectionId: SectionId;
      await act(async () => {
        sectionId = getState().createSection(50, 50, 400, 300, 'Test Section');
      });
    ...
    it('should update section bounds and recalculate containment', async () => {
      ...
      expect(element?.x).toBe(510); // 500 + 10
      expect(element?.y).toBe(510); // 500 + 10
    });
  });

  describe('Element Containment Logic', () => {
    it('should correctly identify if an element is contained within a section', async () => {
      const { getState } = store;
      let sectionId: SectionId;
      await act(async () => {
