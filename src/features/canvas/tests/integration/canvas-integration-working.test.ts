  ...
  describe('🧪 Advanced Integration Scenarios', () => {
    test('Canvas save/load workflow simulation', () => {
      ...
      elements.forEach(el => {
        ...
        // Fix: Create proper history entry objects that match the expected type
        const historyEntry = { 
          type: 'ADD_ELEMENT', 
          elementId: el.id,
          timestamp: Date.now(),
        ...
        (canvasState.history as any[]).push(historyEntry);
      });
    ...
    test('Undo/redo workflow simulation', () => {
      ...
      const historyManager = {
        addToHistory: (action: any) => {
          (canvasState.history as any[]).push(action);
          canvasState.historyIndex = (canvasState.history as any[]).length - 1;
        },
        undo: () => {
          if (canvasState.historyIndex >= 0) {
            const action = (canvasState.history as any[])[canvasState.historyIndex];
        ...
        redo: () => {
          if (canvasState.historyIndex < (canvasState.history as any[]).length - 1) {
            canvasState.historyIndex++;
            const action = (canvasState.history as any[])[canvasState.historyIndex];
            return action;
          }
        ...
        canRedo: () => canvasState.historyIndex < (canvasState.history as any[]).length - 1,
      };
      ...
      expect((canvasState.history as any[]).length).toBe(3);
