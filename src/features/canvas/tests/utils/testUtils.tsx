...
export const createMockCanvasElement = (overrides: any = {}): any => {
  const { ElementId, SectionId } = require('../../types/enhanced.types');
  
  const baseElement = {
    id: ElementId(`test-element-${Math.random().toString(36).substr(2, 9)}`),
    type: 'rectangle',
    x: 100,
    y: 100,
    rotation: 0,
  ...
  if (overrides.type === 'section') {
    return {
      ...baseElement,
      id: SectionId(baseElement.id), // Override with SectionId for sections
      width: 400,
      height: 300,
      title: 'Test Section',
      titleBarHeight: 40,
...
export const createMockSection = (elementCount: number = 3): {
  ...
  const { ElementId, SectionId } = require('../../types/enhanced.types');
  const sectionId = SectionId(`section-${Math.random().toString(36).substr(2, 9)}`);
