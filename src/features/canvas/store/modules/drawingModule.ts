  ...
  return {
    ...
    actions: {
      startDrawing: (tool, point) => {
        ...
        // Create a live draft path and track via DirectRenderer
        try {
          const { getDirectRenderer } = require('../../renderers/DirectRendererContext');
          const dr = getDirectRenderer?.();
          if (dr) {
            // Create or reuse a hidden draft layer and line identified by a stable id
            // For simplicity, we use a single global draft id per session
            (globalThis as any).__canvasDraftId = (globalThis as any).__canvasDraftId || `draft-path`;
          }
        } catch {}
      },
      ...
      updateDrawing: (point) => {
        ...
        // Attempt to update preview path directly for higher FPS
        try {
          const { getDirectRenderer } = require('../../renderers/DirectRendererContext');
          const dr = getDirectRenderer?.();
          if (dr && path && (globalThis as any).__canvasDraftId) {
            dr.updatePath((globalThis as any).__canvasDraftId, path);
          } else if (dr) {
            dr.requestRedraw();
          }
        } catch {}
      ...
      finishDrawing: () => {
        ...
        if (state.isDrawing && state.currentPath && state.currentPath.length >= 4) {
          ...
          if (tool === 'eraser') {
            // Basic eraser semantics: remove intersected stroke elements
            const size = get().strokeConfig.eraser.size || 30;
            try { (get() as any).eraseInPath?.(state.currentPath, size); } catch {}
          } else if (tool === 'marker') {
            const cfg = get().strokeConfig.marker;
            const markerElement = {
              id: nanoid(),
              type: 'marker',
              x: 0,
              ...
              style: {
                color: cfg.color,
                width: Math.max(cfg.minWidth, Math.min(cfg.maxWidth, cfg.maxWidth)),
                opacity: cfg.opacity,
            ...
            get().addElement(markerElement);
          } else if (tool === 'highlighter') {
            const cfg = get().strokeConfig.highlighter;
            const highlighterElement = {
              ...
              style: {
                ...
                lineCap: 'round',
                lineJoin: 'round',
                blendMode: cfg.blendMode || 'multiply',
            ...
            get().addElement(highlighterElement);
          } else {
            // Default to pen/pencil
            const penColor = get().penColor || '#000000';
            const strokeWidth = tool === 'pencil' ? 1.5 : 2;
            const penElement = {
              id: nanoid(),
              type: 'pen',
              x: 0,
            ...
            get().addElement(penElement);

            // Check sticky note container
