...
const UnifiedTextElement: React.FC<UnifiedTextElementProps> = ({
  ...
  // Temporary test mode - force show menu
  ...
  const startEditing = useCallback(() => {
    ...
    console.log(`[UnifiedTextElement] Setting showFormatMenu to true`);
    setIsEditing(true);
    ...
    setTimeout(() => {
      console.log(`[UnifiedTextElement] Calculating positions...`);
      calculatePositions();
      ...
      setTimeout(() => {
        ...
        console.log(`[UnifiedTextElement] Menu states:`, {
          showFormatMenu,
          menuPosition,
          textareaPosition,
          isEditing
  ...
  }, [element.text, element.id, element.type, calculatePositions, showFormatMenu, menuPosition, textareaPosition, isEditing]);
  ...
  return (
    <>
      {/* Konva elements only - NO DOM elements here */}
      {renderKonvaElements()}
    </>
  );
};

// Separate component for DOM portals - completely outside Konva tree
const UnifiedTextElementPortals: React.FC<{
  isEditing: boolean;
  showFormatMenu: boolean;
  textareaPosition: { x: number; y: number; width: number; height: number } | null;
  menuPosition: { y: number } | null;
  editText: string;
  previewFormat: TextFormat;
  onEditTextChange: (text: string) => void;
  onCancel: () => void;
  onDone: () => void;
  onFormatChange: (formatUpdates: any) => void;
}> = ({
  isEditing,
  showFormatMenu,
  textareaPosition,
  ...
  console.log('🔥🔥🔥 [UnifiedTextElementPortals] Rendering portals:', {
    isEditing,
    showFormatMenu,
    textareaPosition,
    menuPosition
  ...
  return (
    <>
      {/* Test portal - should always render */}
      {createPortal(
        <div style={{
          position: 'fixed',
          top: '50px',
...
// Enhanced UnifiedTextElement with external portal management
const UnifiedTextElementWithPortals: React.FC<UnifiedTextElementProps> = (props) => {
  const [isEditing, setIsEditing] = useState(false);
  const [editText, setEditText] = useState(props.element.text || '');
  const [showFormatMenu, setShowFormatMenu] = useState(true); // Force show for testing
  const [textareaPosition, setTextareaPosition] = useState<{
    x: number;
    y: number;
    width: number;
    height: number;
  } | null>({ x: 100, y: 100, width: 300, height: 150 }); // Default position for testing
  const [menuPosition, setMenuPosition] = useState<{
    y: number;
  } | null>({ y: 300 }); // Default position for testing
  const [previewFormat, setPreviewFormat] = useState<TextFormat>({
    bold: props.element.fontStyle?.includes('bold') || false,
    italic: props.element.fontStyle?.includes('italic') || false,
    underline: props.element.textDecoration?.includes('underline') || false,
    strikethrough: props.element.textDecoration?.includes('line-through') || false,
    fontSize: props.element.fontSize || 16,
    color: props.element.fill || props.element.textColor || '#000000',
    fontFamily: props.element.fontFamily || 'Inter, sans-serif',
    listType: (props.element.listType as 'none' | 'bullet' | 'numbered') || 'none',
    isHyperlink: props.element.isHyperlink || false,
    hyperlinkUrl: props.element.hyperlinkUrl || ''
  });

  return (
    <>
      {/* Konva component */}
      <UnifiedTextElement {...props} />
      
      {/* Portals rendered separately */}
      <UnifiedTextElementPortals
        isEditing={isEditing}
        ...
        onFormatChange={(formatUpdates) => {
          Object.entries(formatUpdates).forEach(([key, value]) => {
            setPreviewFormat(prev => ({ ...prev, [key]: value }));
          });
        }}
      />
    </>
...
export default UnifiedTextElementWithPortals;
