...
describe('Canvas Selection Workflow', () => {
  ...
  it('should create elements and handle selection correctly', () => {
    ...
    expect(state1.elements.get('test-rect-1' as ElementId)).toEqual(testElement);
    ...
    expect(state2.selectedElementIds.has('test-rect-1' as ElementId)).toBe(true);
    expect(state2.selectedElementIds.size).toBe(1);
    expect(state2.lastSelectedElementId).toBe('test-rect-1' as ElementId);
  });
  ...
  it('should handle multi-selection correctly', () => {
    ...
    expect(state2.selectedElementIds.has('rect-1' as ElementId)).toBe(true);
    expect(state2.selectedElementIds.has('circle-1' as ElementId)).toBe(true);
  });
  ...
  it('should handle selection toggle in multi-select mode', () => {
    ...
    expect(store.getState().selectedElementIds.has('sticky-1' as ElementId)).toBe(true);
    expect(store.getState().selectedElementIds.size).toBe(1);
    
    // Toggle deselect (multi-select same element again)
    actions.selectElement('sticky-1' as ElementId, true);
    const finalState = store.getState();
    expect(finalState.selectedElementIds.has('sticky-1' as ElementId)).toBe(false);
    expect(finalState.selectedElementIds.size).toBe(0);
  });
