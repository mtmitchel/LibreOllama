...
describe('Selection State Synchronization Test', () => {
  ...
  test('should synchronize single element selection', async () => {
    ...
    await act(async () => {
      state.selectElement(element1.id);
    });
    
    let currentState = testStore.getState();
    expect(currentState.selectedElementIds.has(element1.id)).toBe(true);
    expect(currentState.selectedElementIds.has(element2.id)).toBe(false);
    expect(currentState.selectedElementIds.size).toBe(1);
    expect(currentState.hasSelection()).toBe(true);
    expect(currentState.isElementSelected(element1.id)).toBe(true);
    expect(currentState.isElementSelected(element2.id)).toBe(false);
    
    // Select second element (should replace first)
    await act(async () => {
      state.selectElement(element2.id);
    });
    
    currentState = testStore.getState();
    expect(currentState.selectedElementIds.has(element1.id)).toBe(false);
    expect(currentState.selectedElementIds.has(element2.id)).toBe(true);
    expect(currentState.selectedElementIds.size).toBe(1);
    expect(currentState.isElementSelected(element1.id)).toBe(false);
    expect(currentState.isElementSelected(element2.id)).toBe(true);
  });
  ...
  test('should synchronize multi-element selection', async () => {
    ...
    await act(async () => {
      state.selectMultipleElements([element1.id, element2.id], true);
    });
    
    let currentState = testStore.getState();
    expect(currentState.selectedElementIds.has(element1.id)).toBe(true);
    expect(currentState.selectedElementIds.has(element2.id)).toBe(true);
    expect(currentState.selectedElementIds.has(element3.id)).toBe(false);
    expect(currentState.selectedElementIds.size).toBe(2);
    expect(currentState.hasSelection()).toBe(true);
    
    // Add to selection
    await act(async () => {
      state.selectMultipleElements([element3.id], false); // addToSelection = false means add
    });
    
    currentState = testStore.getState();
    expect(currentState.selectedElementIds.has(element1.id)).toBe(true);
    expect(currentState.selectedElementIds.has(element2.id)).toBe(true);
    expect(currentState.selectedElementIds.has(element3.id)).toBe(true);
  ...
  test('should synchronize element deselection', async () => {
    ...
    await act(async () => {
      state.deselectElement(element1.id);
    });
  ...
  test('should synchronize toggle selection', async () => {
    ...
    expect(state.selectedElementIds.has(element1.id)).toBe(false);
    
    // Toggle on
    await act(async () => {
      state.toggleElementSelection(element1.id);
    });
  ...
  test('should synchronize clear selection', async () => {
    ...
    await act(async () => {
      state.selectMultipleElements([element1.id, element2.id, element3.id], true);
    });
  ...
  test('should maintain selection state consistency across operations', async () => {
    ...
    await act(async () => {
      ...
      state.toggleElementSelection(element2.id);
    ...
    expect(selectedIds).toContain(element1.id);
    expect(selectedIds).toContain(element2.id);
