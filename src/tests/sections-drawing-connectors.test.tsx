...
// Advanced mock store with detailed section and connector logic
const createAdvancedMockStore = () => ({
  ...
  // Drawing states with detailed tracking
  isDrawing: false,
  ...
  connectorRouting: new Map<ElementId, { path: number[]; waypoints: { x: number; y: number }[] }>(),
  connectorAttachments: new Map<ElementId, { start?: ElementId; end?: ElementId }>(),
  dynamicConnectorUpdates: new Set<ElementId>(),
  ...
  startDrawing: vi.fn((point: { x: number; y: number }, tool: string) => {
    const store = createAdvancedMockStore();
  ...
  // Section Operations with Advanced Logic
  ...
  updateSectionBounds: vi.fn((sectionId: SectionId, newBounds: { x: number; y: number; width: number; height: number }) => {
    ...
    // Trigger containment recalculation
    store.recalculateContainment(sectionId);
    return newBounds;
  }),
  
  // Element Containment Logic
  checkElementContainment: vi.fn((elementId: ElementId, elementBounds: { x: number; y: number; width: number; height: number }): SectionId | null => {
    const store = createAdvancedMockStore();
    // Mock containment logic: check if element is fully within any section
  ...
  recalculateContainment: vi.fn((sectionId: SectionId) => {
    // Mock: Recalculate which elements are contained in section after bounds change
    const store = createAdvancedMockStore();
    ...
    // Check each element for containment
  ...
  createConnector: vi.fn((connectorId: ElementId, startElement: ElementId, endElement: ElementId) => {
    ...
    store.connectorRouting.set(connectorId, { path, waypoints: [] });
  ...
  updateConnectorPath: vi.fn((connectorId: ElementId) => {
    ...
    store.connectorRouting.set(connectorId, { path: newPath, waypoints: [] });
    
    return updatedConnector;
  }),
  
  calculateConnectorPath: vi.fn((start: { x: number; y: number }, end: { x: number; y: number }) => {
    // Mock path calculation - in real implementation this would use pathfinding
  ...
  updateConnectorsForMovedElement: vi.fn((elementId: ElementId) => {
    ...
    // Find all connectors attached to this element
    for (const [connectorId, attachments] of store.connectorAttachments.entries()) {
      if (attachments.start === elementId || attachments.end === elementId) {
        ...
        store.dynamicConnectorUpdates.add(connectorId);
      }
    }
    
    // Update all affected connectors
    affectedConnectors.forEach(connectorId => {
  ...
  getElementBounds: vi.fn((elementId: ElementId) => {
    const store = createAdvancedMockStore();
    const element = store.elements.get(elementId);
  ...
  getSectionContainedElements: vi.fn((sectionId: SectionId): Set<ElementId> => {
    const store = createAdvancedMockStore();
    return store.sectionContainmentCache.get(sectionId) || new Set();
  }),
  
  getConnectorsForElement: vi.fn((elementId: ElementId): ElementId[] => {
    const store = createAdvancedMockStore();
    const connectors: ElementId[] = [];
  ...
  screenToCanvas: vi.fn((point: { x: number; y: number }) => {
    const store = createAdvancedMockStore();
    return {
      x: (point.x - store.viewport.pan.x) / store.viewport.zoom,
      y: (point.y - store.viewport.pan.y) / store.viewport.zoom
    };
  }),
  
  canvasToScreen: vi.fn((point: { x: number; y: number }) => {
    const store = createAdvancedMockStore();
    return {
      x: point.x * store.viewport.zoom + store.viewport.pan.x,
      y: point.y * store.viewport.zoom + store.viewport.pan.y
    };
  }),
});
...
describe('Sections, Containment, Drawing & Dynamic Connectors', () => {
  ...
  describe('🏗️ Section Creation & Management', () => {
    ...
    test('Section collapse/expand affects contained element visibility', () => {
      ...
      // Should call updateElement for each contained element to set visible: false
      ...
      // Should call updateElement again for each contained element to set visible: true
  ...
  describe('📦 Element Containment Logic', () => {
    ...
    test('Containment recalculation after section resize', () => {
      const sectionId = SectionId('dynamic-section');
      const elementId = ElementId('contained-element');
      
      // Create section and element
      mockStore.createSection(sectionId, { x: 100, y: 100, width: 200, height: 150 });
      
      // Add element to store at position that will be affected by resize
      const element = createMockElement('rectangle', {
        id: elementId,
        x: 250, // Initially outside
        y: 200,
        width: 50,
        height: 50
      });
      ...
      // Resize section to include the element
      mockStore.updateSectionBounds(sectionId, { x: 100, y: 100, width: 300, height: 200 });
      
      // Should trigger recalculation
      expect(mockStore.recalculateContainment).toHaveBeenCalledWith(sectionId);
      
      // Manually call recalculation to test logic
      const newContained = mockStore.recalculateContainment(sectionId);
      
      expect(newContained).toBeInstanceOf(Set);
    });
  });
  ...
  describe('🔗 Dynamic Connector System', () => {
    ...
    test('Dynamic connector path updates when elements move', () => {
      ...
      // Get initial path
      const initialPath = connector.pathPoints;
      ...
      expect(mockStore.dynamicConnectorUpdates.has(connectorId)).toBe(true);
  ...
  describe('🔄 Complex Integration Scenarios', () => {
    ...
    test('Section collapse hides elements but maintains connector routing', () => {
      const sectionId = SectionId('collapsing-section');
      const containedElementId = ElementId('hidden-element');
      const externalElementId = ElementId('visible-element');
      const connectorId = ElementId('persistent-connector');
      
      // Setup section with contained element
      mockStore.createSection(sectionId, { x: 100, y: 100, width: 200, height: 200 });
      
      mockStore.elements.set(containedElementId, createMockElement('rectangle', {
        id: containedElementId,
        x: 150,
        y: 150
      }));
      ...
      mockStore.moveElementToSection(containedElementId, sectionId);
      
      // Create connector
      mockStore.createConnector(connectorId, containedElementId, externalElementId);
      
      // Collapse section
      mockStore.collapseSection(sectionId);
      
      expect(mockStore.collapseSection).toHaveBeenCalledWith(sectionId);
      expect(mockStore.updateElement).toHaveBeenCalledWith(containedElementId, { visible: false });
      
      // Connector should still exist and be routable
      const connector = mockStore.connectors.get(connectorId);
      expect(connector).toBeDefined();
      expect(connector?.startElementId).toBe(containedElementId);
      expect(connector?.endElementId).toBe(externalElementId);
    });

    test('Nested sections with cross-level connectors', () => {
      const parentSectionId = SectionId('parent-section');
      const childSectionId = SectionId('child-section');
      const parentElementId = ElementId('parent-element');
      const childElementId = ElementId('child-element');
      const nestedConnectorId = ElementId('nested-connector');
      
      // Create parent section
      mockStore.createSection(parentSectionId, { x: 50, y: 50, width: 400, height: 300 });
      
      // Create child section inside parent
      mockStore.createSection(childSectionId, { x: 100, y: 100, width: 150, height: 100 });
      mockStore.moveElementToSection(childSectionId as any, parentSectionId);
      
      // Create elements in different section levels
      mockStore.elements.set(parentElementId, createMockElement('rectangle', {
        id: parentElementId,
        x: 300, y: 200 // In parent section but outside child
      }));
      
      mockStore.elements.set(childElementId, createMockElement('circle', {
        id: childElementId,
        x: 125, y: 125 // In child section
      }));
      
      mockStore.moveElementToSection(parentElementId, parentSectionId);
      mockStore.moveElementToSection(childElementId, childSectionId);
      
      // Create connector across section levels
      mockStore.createConnector(nestedConnectorId, childElementId, parentElementId);
      
      expect(mockStore.createConnector).toHaveBeenCalledWith(nestedConnectorId, childElementId, parentElementId);
      
      // Verify connector routing works across nested sections
      const connector = mockStore.connectors.get(nestedConnectorId);
      expect(connector).toBeDefined();
      expect(connector?.pathPoints).toBeDefined();
    });
  });
  ...
  describe('🎯 Performance & Edge Cases', () => {
    ...
    test('Multiple connectors with complex routing updates', () => {
      const centralElementId = ElementId('routing-hub');
      ...
      // Create many connected elements and connectors
      for (let i = 0; i < connectorCount; i++) {
        const spokeElementId = ElementId(`spoke-${i}`);
        const connectorId = ElementId(`connector-${i}`);
        
        const angle = (i / connectorCount) * 2 * Math.PI;
        const radius = 200;
        const x = 400 + Math.cos(angle) * radius;
        const y = 300 + Math.sin(angle) * radius;
        
        mockStore.elements.set(spokeElementId, createMockElement('circle', {
          id: spokeElementId,
          x,
          y
        }));
        
        mockStore.createConnector(connectorId, centralElementId, spokeElementId);
      }
      
      const startTime = performance.now();
      
      // Move central element (should update all connectors)
      mockStore.moveElement(centralElementId, { x: 450, y: 350 });
      const affectedConnectors = mockStore.updateConnectorsForMovedElement(centralElementId);
      
      const endTime = performance.now();
      const operationTime = endTime - startTime;
      
      expect(operationTime).toBeLessThan(500); // Should handle many connectors efficiently
      expect(affectedConnectors).toHaveLength(connectorCount);
    });
    ...
    test('Edge case: Element on exact section boundary', () => {
      ...
      const outsideElementBounds = { x: 251, y: 251, width: 50, height: 50 };
