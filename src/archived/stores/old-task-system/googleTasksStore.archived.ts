        ...
        createTask: async (taskListId: string, taskData: TaskCreateData) => {
          ...
          try {
            ...
            if (response.success && response.data) {
              ...
              // ✅ CRITICAL FIX: Return the created task
        ...
        deleteTask: async (taskListId: string, taskId: string) => {
          ...
          try {
            ...
            if (response.success) {
              ...
              const { deleteTaskMetadata } = await import('./taskMetadataStore').then(m => m.useTaskMetadataStore.getState());
              deleteTaskMetadata(taskId);
              
              logger.debug(`[GOOGLE-TASKS] Task ${taskId} deleted and metadata cleaned up`);
            } else {
...
export async function initializeGoogleTasksStore() {
  logger.debug('[GOOGLE-TASKS] Initializing store...');
